...
import io.kuark.ability.sys.provider.reg.dao.RegResourceDao
...
import io.kuark.ability.sys.provider.reg.model.table.RegResources
...
//endregion your codes 1
    ...
    private fun recursionFindAllParentId(itemId: String, results: MutableList<String>) {
        val list = dao.oneSearchProperty(RegResources.id.name, itemId, RegResources.parentId.name)
        if (list.isNotEmpty()) {
    ...
    private fun recursionFindAllChildId(resId: String, results: MutableList<String>) {
        val itemIds = dao.oneSearchProperty(RegResources.parentId.name, resId, RegResources.id.name)
        itemIds.forEach { id ->
