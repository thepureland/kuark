...
import org.kuark.base.bean.validation.constraint.validator.NotBlankOnValidator
...
/**
 * 动态NotBlank约束注解，属性级别注解。
 *
 * 当前属性的值是否可以为空白，取决于定义的表达式。表达式为false，属性值可为空白；表达式为true，属性值不可为空白。
 *
 * @author K
 * @since 1.0.0
 */
@Constraint(validatedBy = [NotBlankOnValidator::class])
@Target(AnnotationTarget.FUNCTION, AnnotationTarget.PROPERTY_GETTER, AnnotationTarget.PROPERTY_SETTER)
@Retention(AnnotationRetention.RUNTIME)
@MustBeDocumented
@Repeatable
annotation class NotBlankOn(

    /** NotBlank约束依赖的前提条件, 条件成立等效于@NotBlank，条件不成立时等效于没有此约束注解 */
    val depends: Depends,
    ...
    @Retention(AnnotationRetention.RUNTIME)
    @MustBeDocumented
    annotation class List(vararg val value: NotBlankOn)
