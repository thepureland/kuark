...
import org.kuark.base.bean.validation.constraint.validator.NotEmptyOnValidator
...
/**
 * 动态NotEmpty约束注解，属性级别注解。
 *
 * 当前属性的值是否可以为空，取决于定义的表达式。表达式为false，属性值可为空，即非必填选项；表达式为true，属性值必填。
 *
 * @author K
 * @since 1.0.0
 */
@Constraint(validatedBy = [NotEmptyOnValidator::class])
@Target(AnnotationTarget.FUNCTION, AnnotationTarget.PROPERTY_GETTER, AnnotationTarget.PROPERTY_SETTER)
@Retention(AnnotationRetention.RUNTIME)
@MustBeDocumented
@Repeatable
annotation class NotEmptyOn(

    /** NotEmpty约束依赖的前提条件, 条件成立等效于@NotEmpty，条件不成立时等效于没有此约束注解 */
    val depends: Depends,
    ...
    @Retention(AnnotationRetention.RUNTIME)
    @MustBeDocumented
    annotation class List(vararg val value: NotEmptyOn)
