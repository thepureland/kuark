...
class AppendCodesRetriever(private val fileContent: String?) {
    ...
    fun retrieve(): Map<Int, Pair<io.kuark.demo.tools.codegen.core.merge.AppendCodeType, String>> {
        val map = hashMapOf<Int, Pair<io.kuark.demo.tools.codegen.core.merge.AppendCodeType, String>>()
        val p =
            Pattern.compile("(?<=(<!--)?#?//region append (\\w{1,10}) codes (\\d)(-->)?\\r?\\n)[\\s\\S]*?(?=(<!--)?#?//endregion append \\w+ codes \\d(-->)?)")
        val m = p.matcher(fileContent)
        while (m.find()) {
            map[m.group(3).toInt()] = Pair(io.kuark.demo.tools.codegen.core.merge.AppendCodeType.valueOf(m.group(2)), m.group(0))
        }
        return map
    }
