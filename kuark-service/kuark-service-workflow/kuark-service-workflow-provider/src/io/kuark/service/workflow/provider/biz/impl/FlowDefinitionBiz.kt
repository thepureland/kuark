package io.kuark.service.workflow.provider.biz.impl
...
open class FlowDefinitionBiz : IFlowDefinitionBiz {
    ...
    override fun search(searchPayload: FlowDefinitionSearchPayload): List<FlowDefinition> {
        ...
        if (CollectionKit.isNotEmpty(orders)) {
            val orderStr = RdbKit.getOrderSql(*orders!!.toTypedArray())
            sql = "$sql $orderStr"
        }

        // 查询流程定义
        val query = repositoryService.createNativeModelQuery().sql(sql)
        val pageSize = searchPayload.pageSize
        val pageNum = searchPayload.pageNo
        val models = if (pageNum == null || pageSize == null || pageSize < 1) {
            query.list()
        } else {
